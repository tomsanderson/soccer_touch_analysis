...
Decide which template it matches:
...
Build an event dict matching Schema_V1, including:
...
if "on target" in clause_lower:
    ...
    # next_possession_team / action_outcome_team can be chosen for V1
...
Core fields (event_id, match_id, period, video_time_s) come from context.
...
Use a simple possession counter per half:

current_possession_id = 1


Assign possession_id = str(current_possession_id) to events until a possession ends.

A possession may be treated as “ended” when:

A first_touch event has maintained_possession_bool == False and possession_after_touch == "opponent", or

An on_ball_action event has action_outcome_team == "opponent" or action_outcome_team == "out_of_play" with restart belonging to the opponent.
...
Next first_touch event for your team gets the new possession_id.

This is approximate but sufficient for V1.

8. Testing Suggestions

Implement unit tests (e.g., backend/tests/test_parser.py) with canonical examples.
...
Any changes to regex patterns, event routing, or field mapping should be documented in a new file (e.g., Parser_Spec_V2.md) and kept in sync with the corresponding schema/grammar versions.
